...
export default async function handler(req, res) {
  ...
  const { action, nome, email, senha, senha_hash, data_nascimento, sexo, cidade, estado, pais, nome_pai, nome_mae, servico_limitado, tipo_servico } = req.body;
  ...
  if (action === 'iniciar_cadastro') {
    ...
    return res.status(200).json({ success: true, codigo, nome });
  }
  ...
  // ── VERIFICAR ACESSO (email liberado, assinatura com pool/ciclo, ou créditos de trial) ──
  if (action === 'verificar_acesso') {
    // Limite mensal por plano, PARA CADA SERVIÇO SEPARADAMENTE (Consulta, Meditação,
    // Mentoria). null = ilimitado (Plano Infinito). O parâmetro tipo_servico, enviado
    // pelo perfil, diz qual dos três está sendo verificado; servico_limitado indica
    // se o módulo de destino usa limite ou é sempre livre (Frequências, Reflexão, Sonhos).
    ...
    // 1. Verifica a assinatura ativa mais recente e se está dentro do ciclo de 30 dias
    const subResult = await supabaseFetch(
      `/assinaturas?email=eq.${encodeURIComponent(email)}&status=eq.authorized&order=id.desc&limit=1&select=id,servico,ciclo_inicio,usos_consulta,usos_meditacao,usos_mentoria`,
      { method: 'GET' }
    );
    const assinatura = subResult.ok && Array.isArray(subResult.data) ? subResult.data[0] : null;
    const dentroDoCiclo = !!(assinatura && assinatura.ciclo_inicio &&
      (Date.now() - new Date(assinatura.ciclo_inicio).getTime()) < DIAS_CICLO * 24 * 60 * 60 * 1000);

    if (assinatura && dentroDoCiclo) {
      const limites = LIMITES_POR_PLANO[assinatura.servico];

      if (limites) {
        const limite = tipo_servico ? limites[tipo_servico] : null;

        if (!servico_limitado || limite === null || limite === undefined) {
          return res.status(200).json({ acesso: true, tipo: 'assinante' });
        }
        const coluna = 'usos_' + tipo_servico;
        if ((assinatura[coluna] || 0) < limite) {
          return res.status(200).json({ acesso: true, tipo: 'assinante_pool' });
        }
        return res.status(200).json({ acesso: false, motivo: 'pool_esgotado' });
      }
    }
