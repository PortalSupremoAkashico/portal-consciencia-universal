...
async function enviarEmailCodigo(to, nome, codigo) {
  ...
  try {
    await smtpTransporter.sendMail({
      ...
      to,
      subject: 'Código de Verificação — Portal da Consciência Universal',
      text: corpo
    });
    console.log('[auth] código enviado via SMTP para:', to);
  } catch (e) { console.error('[auth] erro ao enviar e-mail de código via SMTP:', e.message); }
}
