...
// Mesmo endpoint usado por /api/email para o fluxo de recuperação de senha
...
async function enviarEmailCodigo(to, nome, codigo) {
  ...
  try {
    ...
    console.log('[auth] envio código GAS status:', resp.status, JSON.stringify(data).slice(0, 150));
  } catch (e) { console.error('[auth] erro ao enviar e-mail de código:', e.message); }
}
