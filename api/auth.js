...
export default async function handler(req, res) {
  ...
  // Também usado pelo perfil.html para exibir o contador de créditos restantes.
  if (action === 'verificar_acesso') {
    // Pool mensal de créditos COMPARTILHADO entre Consulta Akáshica, Mentoria
    // Akáshica, Reflexões, Meditação Guiada, Frequência/Geometria Sagrada e Sonho
    // Analisado. null = ilimitado (Plano Infinito). Fórum Ufológico e Relatos
    // Pessoais NUNCA passam por aqui — são sempre livres para quem se cadastra.
    const POOL_POR_PLANO = { plano_despertar: 5, plano_expansao: 10, plano_infinito: null, portal_mensal: null };
    const DIAS_CICLO = 30;
    ...
    // 1. Verifica assinatura ativa dentro do ciclo de 30 dias e o pool compartilhado
    const subResult = await supabaseFetch(
      `/assinaturas?email=eq.${encodeURIComponent(email)}&status=eq.authorized&order=id.desc&limit=1&select=id,servico,ciclo_inicio,pool_usado`,
      { method: 'GET' }
    );
    ...
    if (assinatura && dentroDoCiclo) {
      ...
      const usado = assinatura.pool_usado || 0;

      if (limite === null || limite === undefined) {
        return res.status(200).json({ acesso: true, tipo: 'assinante', plano: assinatura.servico, limite: null, usado, restante: null });
      }
      if (usado < limite) {
        return res.status(200).json({ acesso: true, tipo: 'assinante', plano: assinatura.servico, limite, usado, restante: limite - usado });
      }
      return res.status(200).json({ acesso: false, tipo: 'cota_esgotada', plano: assinatura.servico, limite, usado, restante: 0 });
    }
    ...
    return res.status(200).json({ acesso: false, tipo: 'sem_credito' });
  }
