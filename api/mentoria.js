...
const TIPO_SERVICO = 'mentoria'; // este arquivo é sempre Mentoria Akáshica
const DIAS_CICLO = 30;
...
async function debitarCredito(email, motivo) {
  const resp = await fetch(`${SUPABASE_URL}/rest/v1/creditos_movimentos`, {
    ...
    body: JSON.stringify({ email, quantidade: -1, motivo, referencia: `mentoria_${Date.now()}` })
  });
  if (!resp.ok) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const acesso = await verificarAcesso(user?.email, true, 'mentoria');
    if (!acesso.permitido) {
...
- PROIBIDO usar travessão (—) em qualquer parte da resposta. Use vírgula, ponto, dois-pontos ou ponto-e-vírgula no lugar
...
SEMPRE: termine com 1 pergunta poderosa que ${firstName} vai carregar — não mais que uma.`;
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: process.env.ANTHROPIC_MODEL_OPUS || 'claude-opus-4-8',
        max_tokens: 16000,
        system: systemPrompt,
        messages,
        stream: true
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (ev.type === 'content_block_delta' && ev.delta?.type === 'text_delta' && ev.delta.text) {
            ...
            res.write(ev.delta.text.replace(/—/g, ','));
          }
        } catch {}
      }
    }
