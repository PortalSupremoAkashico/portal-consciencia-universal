...
async function temAssinaturaAtiva(email) {
  const url = `${SUPABASE_URL}/rest/v1/assinaturas?email=eq.${encodeURIComponent(email)}&servico=eq.mentoria&status=eq.authorized&select=id`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const emailLiberado = !!user?.email && EMAILS_LIBERADOS.includes(String(user.email).toLowerCase().trim());
    if (!emailLiberado) {
      if (!user?.email) return res.status(400).json({ error: 'Email obrigatório.' });
      const ativa = await temAssinaturaAtiva(user.email);
      if (!ativa) {
        return res.status(402).json({ error: 'Sua assinatura da Mentoria não está ativa. Assine em /planos para continuar.' });
      }
    }
