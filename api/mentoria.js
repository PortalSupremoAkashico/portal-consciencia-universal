...
async function temAssinaturaPortal(email) {
  const url = `${SUPABASE_URL}/rest/v1/assinaturas?email=eq.${encodeURIComponent(email)}&servico=in.(plano_despertar,plano_expansao,plano_infinito,portal_mensal)&status=eq.authorized&select=id`;
  const resp = await fetch(url, { headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` } });
  const rows = await resp.json().catch(() => []);
  return Array.isArray(rows) && rows.length > 0;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const emailLiberado = !!user?.email && await emailEhLiberado(user.email);
    const assinante = !emailLiberado && !!user?.email && await temAssinaturaPortal(user.email);
    if (!emailLiberado && !assinante) {
      if (!user?.email) return res.status(400).json({ error: 'Email obrigatório.' });
      const saldo = await obterSaldoCreditos(user.email);
      if (saldo < 1) {
    ...
    // ── Dados biográficos complementares (hora/local de nascimento, pais) ──
    let bioContext = '';
    if (user?.cidade || user?.estado || user?.pais) bioContext += `\nLOCAL DE NASCIMENTO: ${[user?.cidade, user?.estado, user?.pais].filter(Boolean).join(', ')}`;
    if (user?.hora_nascimento) bioContext += `\nHORA DE NASCIMENTO: ${user.hora_nascimento}`;
    if (user?.nome_pai) bioContext += `\nNOME DO PAI: ${user.nome_pai}`;
    if (user?.nome_mae) bioContext += `\nNOME DA MÃE: ${user.nome_mae}`;
    const bioContextBlock = bioContext
      ? `\nDADOS BIOGRÁFICOS COMPLEMENTARES DE ${firstName.toUpperCase()} (use com sutileza — apenas onde agregar profundidade real à sessão; NUNCA liste ou mencione mecanicamente todos os dados, e NUNCA cite nome dos pais ou local de nascimento de forma repetida ao longo da conversa):${bioContext}\n`
      : '';
...
SEMPRE: termine com 1 pergunta poderosa que ${firstName} vai carregar — não mais que uma.`;
    ...
    if (!emailLiberado && !assinante) {
      await debitarCredito(user.email, 'mentoria').catch(() => {});
    }
