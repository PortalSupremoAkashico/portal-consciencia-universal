...
export default async function handler(req, res) {
  // ── GET — a página consulta periodicamente se o Pix já foi confirmado ──
  if (req.method === 'GET') {
    const { external_reference } = req.query || {};
    if (!external_reference) return res.status(400).json({ erro: 'external_reference obrigatório' });
    try {
      const url = `${SUPABASE_URL}/rest/v1/assinaturas?external_reference=eq.${encodeURIComponent(external_reference)}&select=status`;
      const resp = await fetch(url, {
        headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` }
      });
      const linhas = await resp.json().catch(() => []);
      const status = Array.isArray(linhas) && linhas[0] ? linhas[0].status : 'pending';
