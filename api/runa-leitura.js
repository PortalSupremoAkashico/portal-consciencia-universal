...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      const path = email === 'all'
        ? `/leituras_runas?order=created_at.desc`
        : `/leituras_runas?email=eq.${encodeURIComponent(email)}&order=created_at.desc`;
      const r = await fetch(
  ...
  if (body.action === 'apagar') {
    ...
    try {
      await fetch(
        `${SUPABASE_URL}/rest/v1/leituras_runas?email=eq.${encodeURIComponent(email)}`,
  ...
  const prompt = `Você é um Mestre Rúnico de elite, profundo conhecedor do Elder Futhark e da sabedoria ancestral nórdica. Leitura rúnica profunda e altamente personalizada para ${firstName}.

TRADIÇÃO: Elder Futhark — cada runa é um símbolo sagrado vivo, portal para a sabedoria dos Ancestrais nórdicos, carregando memória cósmica gravada desde o início dos tempos.
...
- Mergulhe na tradição nórdica — cite o mito, o elemento, a divindade e a energia de cada runa
...
§1: O que esta runa carrega — sua memória ancestral nórdica, seu mito, seu elemento e sua vibração única
§2: Como esta energia se manifesta ESPECIFICAMENTE para você (${firstName}) nesta posição em relação a "${perguntaReal}"
§3: A mensagem dos Ancestrais e das runas para você — orientação concreta, amorosa e acionável

SINTESE: 3 parágrafos — (1) padrão rúnico que emerge do conjunto das runas tiradas; (2) verdade central que as runas revelam para ${firstName} sobre "${perguntaReal}"; (3) caminho indicado pelas runas com clareza e confiança
...
{
  "titulo": "título poético nórdico desta leitura específica",
  ${runasJson},
  "sintese": "...",
  "acao_sagrada": "..."
}`;
  ...
  try {
    ...
    if (!emailLiberado && !assinante && email) {
      await debitarCredito(email, 'uso_runas').catch(() => {});
    }
