      ...
      if (pagamento.external_reference) {
        ...
        if (pagamento.status === 'approved') {
          ...
          if (registro && !(await jaCreditado(pagamento.external_reference))) {
            // Pacotes concedem múltiplos créditos; serviços avulsos concedem 1
            const quantidade = PACOTES_CREDITOS[registro.servico] || 1;
            const motivo = PACOTES_CREDITOS[registro.servico]
              ? `compra_pacote_${quantidade}`
              : `compra_${registro.servico}`;
            await creditarPacote(registro.email, quantidade, motivo, pagamento.external_reference);
          }
        }
      }
    }
      ...
      if (cobranca.preapproval_id && cobranca.payment && cobranca.payment.status === 'approved') {
        ...
        if (creditosPorCiclo) {
          ...
          if (!(await jaCreditado(referenciaCiclo))) {
            await creditarPacote(assinaturaRegistro.email, creditosPorCiclo, `ciclo_${assinaturaRegistro.servico}`, referenciaCiclo);
          }
        }
      }
    }
