  ...
  try {
    ...
    if (type === 'payment') {
      ...
      if (pagamento.external_reference) {
        ...
        if (pagamento.status === 'approved') {
          ...
          // Plano pago via Pix (pagamento único, não recorrente).
          // Se o external_reference não bater com nenhuma assinatura, esse PATCH
          // simplesmente não afeta nenhuma linha — inofensivo pros outros fluxos.
          await atualizarSupabase('assinaturas', 'external_reference', pagamento.external_reference, {
            status: 'authorized',
          ...
          // Pix não pago (expirou ou foi rejeitado) — encerra a assinatura pendente
          await atualizarSupabase('assinaturas', 'external_reference', pagamento.external_reference, {
            status: 'cancelled',
            atualizado_em: new Date().toISOString()
          });
    ...
    if (type === 'subscription_preapproval') {
      ...
      if (assinatura.status === 'authorized') {
        ...
        atualizacao.usos_consulta = 0;
        atualizacao.usos_meditacao = 0;
        atualizacao.usos_mentoria = 0;
      }
