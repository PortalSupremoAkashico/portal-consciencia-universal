...
// Limite mensal por plano, PARA CADA SERVIÇO SEPARADAMENTE (Consulta, Meditação, Mentoria).
// null = ilimitado (Plano Infinito). Frequências/Reflexão/Sonhos não usam limite:
// para esses, qualquer assinatura ativa dentro do ciclo já libera de graça.
const LIMITES_POR_PLANO = {
  plano_despertar: { consulta: 1, meditacao: 1, mentoria: 1 },
  plano_expansao:  { consulta: 3, meditacao: 3, mentoria: 3 },
  plano_infinito:  { consulta: null, meditacao: null, mentoria: null },
  portal_mensal:   { consulta: 1, meditacao: 1, mentoria: 1 } // plano legado, antes dos 3 planos atuais — mesmo limite do Despertar
};
const TIPO_SERVICO = 'consulta'; // este arquivo é sempre Consulta Akáshica
const DIAS_CICLO = 30;
...
async function buscarAssinaturaAtiva(email) {
  const url = `${SUPABASE_URL}/rest/v1/assinaturas?email=eq.${encodeURIComponent(email)}&status=eq.authorized&order=id.desc&limit=1&select=id,servico,ciclo_inicio,usos_consulta,usos_meditacao,usos_mentoria`;
  const resp = await fetch(url, { headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` } });
  const rows = await resp.json().catch(() => []);
...
async function incrementarUso(assinaturaId, coluna, valorAtual) {
  const resp = await fetch(`${SUPABASE_URL}/rest/v1/assinaturas?id=eq.${assinaturaId}`, {
    method: 'PATCH',
    headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}`, 'Content-Type': 'application/json', Prefer: 'return=minimal' },
    body: JSON.stringify({ [coluna]: valorAtual + 1 })
  });
  if (!resp.ok) {
    const errTxt = await resp.text().catch(() => '');
    console.error('incrementarUso falhou:', resp.status, errTxt.slice(0,300));
  }
}

// Verifica acesso e devolve uma função "cobrar" a ser chamada só depois que a
// IA confirmar que vai responder (nunca cobra antes de saber que o serviço
// foi de fato prestado). servicoLimitado=true usa o pool mensal do plano.
async function verificarAcesso(email, servicoLimitado, motivo) {
  ...
  if (assinatura && dentroDoCiclo(assinatura.ciclo_inicio)) {
    const limites = LIMITES_POR_PLANO[assinatura.servico];

    // Plano não reconhecido: por segurança, NÃO libera sem limite — cai pro
    // sistema de créditos abaixo, em vez de tratar como ilimitado.
    if (limites) {
      const limite = limites[TIPO_SERVICO];

      if (!servicoLimitado || limite === null || limite === undefined) {
        return { permitido: true, cobrar: async () => {} };
      }

      const coluna = 'usos_' + TIPO_SERVICO;
      const usoAtual = assinatura[coluna] || 0;
      if (usoAtual < limite) {
        return { permitido: true, cobrar: async () => { await incrementarUso(assinatura.id, coluna, usoAtual); } };
      }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── Verificação de acesso: email liberado, pool do plano, ou crédito de trial ──
    const acesso = await verificarAcesso(email, true, 'uso_consulta');
    if (!acesso.permitido) {
      res.setHeader('Content-Type', 'application/json; charset=utf-8');
...
- Cada seção JSON deve ter entre 400-500 palavras — denso e profundo, sem repetição entre seções
...
${baseSystemPrompt}`;
    ...
    // Cobra (incrementa pool do plano ou debita crédito de trial) assim que a IA
    // confirma que vai responder — antes do streaming, para não perder o débito
    // caso a função seja interrompida por timeout (consulta é o serviço mais longo).
