...
ACTION (Ação Sagrada):
- 3 ações CONCRETAS, ESPECÍFICAS e PERSONALIZADAS para os próximos 30 dias — cada uma nasce diretamente da pergunta, do estado emocional e da revelação desta leitura. NADA que qualquer pessoa poderia fazer
- Cada ação tem um "porquê" — explique por que ESTA ação específica, por que agora, como ela trabalha o padrão revelado
- Uma prática espiritual personalizada — ritual, intenção ou âncora energética que nasce especificamente desta leitura, não de um manual
- A pergunta-semente que ${firstName} deve carregar consigo: não uma pergunta motivacional genérica, mas a pergunta específica que, se respondida com honestidade, vai abrir o próximo nível da jornada dele/dela
...
Responda APENAS em JSON válido com estas 6 chaves:
{
  ...
  "action": "..."
    ...
    // Pool espiritual com 6 dimensões
    const poolEspiritual = ['espirita', 'cristao', 'akashico', 'egipcio', 'oriental', 'xamanico'];
    // Âncora: 50% espírita, 50% akáshico (identidade do portal)
    const ancora = Math.random() < 0.5 ? 'espirita' : 'akashico';
    // 2ª e 3ª: livres do restante
    const restante = poolEspiritual.filter(p => p !== ancora).sort(() => Math.random() - 0.5);
    const spiritual1 = ancora;
    const spiritual2 = restante[0];
    const spiritual3 = restante[1];

    console.log('🎯 Perspectivas: ' + primary.toUpperCase() + ' + ' + secondary.toUpperCase() + ' | Espiritual: ' + spiritual1.toUpperCase() + ' + ' + spiritual2.toUpperCase() + ' + ' + spiritual3.toUpperCase());
    ...
    const poolMestres = [
      ...mestresPorTradic[spiritual1],
      ...mestresPorTradic[spiritual2],
      ...mestresPorTradic[spiritual3],
...
DIMENSÃO ESPIRITUAL PROFUNDA — 3 TRADIÇÕES COM AUTENTICIDADE:
Você tem acesso a três tradições espirituais nesta consulta. Use-as de forma fluida, complementar e GENUÍNA — nunca force uma referência que não tenha conexão real com a pergunta do consulente. A profundidade de uma tradição bem aplicada vale mais do que três usadas superficialmente. Quando as tradições divergem, apresente as perspectivas com honestidade — isso enriquece. Conecte cada referência ao contexto concreto da pergunta.

TRADIÇÃO ÂNCORA DO PORTAL:
${systemPrompts[spiritual1]}

TRADIÇÃO COMPLEMENTAR 1:
${systemPrompts[spiritual2]}

TRADIÇÃO COMPLEMENTAR 2:
${systemPrompts[spiritual3]}

MESTRES DA HUMANIDADE — cite entre 1 e 3 ao longo da leitura: ${mestresSorteados.join(', ')}.
Use apenas quando a conexão com a pergunta for genuína. 1 mestre bem aplicado vale mais que 3 forçados. A citação deve iluminar a situação de ${firstName}.
