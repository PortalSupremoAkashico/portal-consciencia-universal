...
Fontes prioritárias: AARO (aaro.mil), Pentágono (defense.gov), NASA (nasa.gov), Congresso dos EUA, MUFON (mufon.com), WAR.GOV/UFO (war.gov/ufo), CIA (cia.gov), The Debrief (thedebrief.org), History Channel. Ao buscar, priorize sempre essas fontes.
NUNCA use asterisco (* ou **) em nenhuma resposta, nem para negrito, nem para itálico, nem para lista, nem para separar seções; use apenas ✦ para separar seções. NUNCA use ## em nenhuma resposta.
NUNCA use travessão (—) em nenhuma resposta, em nenhuma hipótese, incluindo na pergunta final para debate; substitua sempre por vírgula, ponto final ou dois-pontos.
...
Ao final, proponha UMA PERGUNTA para debate, em frase direta e sem travessão nem **.
...
IMPORTANTE, SEM EXCEÇÃO: nunca use travessão (—) em nenhuma frase desta resposta, nem use nenhum asterisco (* ou **) em nenhum lugar. Isso vale também para a pergunta final de debate. Se sentir vontade de usar travessão, use vírgula, ponto final ou dois-pontos no lugar.`;
...
export default async function handler(req, res) {
  ...
  try {
    const { pergunta, historico = [], email } = req.body;
    if (!pergunta || !pergunta.trim()) return res.status(400).json({ error: 'Pergunta obrigatória.' });

    // ── Fórum é sempre livre de planos e créditos, mas exige login ──
    if (!email || !String(email).trim()) {
      return res.status(401).json({ error: 'Faça login para participar do Fórum Ufológico.' });
    }
    ...
    const messages = isComentario
      ? [{ role: 'user', content:
          ...
          'uma fonte oficial (NASA, AARO, Pentágono, MUFON, WAR.GOV/UFO), um caso recente ou uma análise crítica. ' +
    ...
    let buf = '';
    let asteriscoPendente = false; // true quando o pedaço anterior terminou com um "*" sozinho, esperando ver se o próximo completa o par "**"

    function filtrarTrecho(t){
      return t
        .replace(/—/g, ',')
        .replace(/\*\*/g, '')
        .replace(/\*/g, '')
        .replace(/\s+,/g, ',')   // tira espaço solto antes de vírgula (sobra comum ao trocar travessão)
        .replace(/,\s*,/g, ','); // junta vírgula duplicada (quando o travessão já estava colado numa vírgula)
    }

    function enviarTexto(novoTrecho){
      let texto = (asteriscoPendente ? '*' : '') + novoTrecho;
      asteriscoPendente = false;
      const finalAsteriscos = texto.match(/\*+$/);
      if (finalAsteriscos && finalAsteriscos[0].length % 2 === 1) {
        // termina em número ímpar de asteriscos: guarda 1 pra ver se o próximo pedaço completa o "**"
        asteriscoPendente = true;
        texto = texto.slice(0, -1);
      }
      const filtrado = filtrarTrecho(texto);
      if (filtrado) res.write(`data: ${JSON.stringify({ type: 'text', text: filtrado })}\n\n`);
    }
