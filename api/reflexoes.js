...
export default async function handler(req, res) {
  ...
  if (req.body?.action === 'salvar' || req.body?.action === 'apagar_um' || req.body?.action === 'apagar' || req.body?.action === 'curtir') {
    const { action, email, dados, id } = req.body;

    if (action === 'curtir') {
      if (!id) return res.status(400).json({ error: 'ID obrigatório.' });
      const r = await sbFetch(`/reflexoes_sessoes?id=eq.${id}&select=dados`);
      if (!r.ok || !Array.isArray(r.data) || !r.data[0]) return res.status(404).json({ error: 'Reflexão não encontrada.' });
      let d; try { d = typeof r.data[0].dados === 'string' ? JSON.parse(r.data[0].dados) : r.data[0].dados; } catch { d = {}; }
      d.likes = (d.likes || 0) + 1;
      const u = await sbFetch(`/reflexoes_sessoes?id=eq.${id}`, { method: 'PATCH', body: JSON.stringify({ dados: JSON.stringify(d) }), headers: { Prefer: 'return=minimal' } });
      return res.status(u.ok ? 200 : 500).json(u.ok ? { success: true, likes: d.likes } : { error: 'Erro ao curtir.' });
    }
