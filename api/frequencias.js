...
// null = ilimitado (Plano Infinito). Frequências não usa pool: qualquer
// assinatura ativa dentro do ciclo já libera de graça.
...
async function buscarAssinaturaAtiva(email) {
  const url = `/assinaturas?email=eq.${encodeURIComponent(email)}&status=eq.authorized&order=id.desc&limit=1&select=id,servico,ciclo_inicio,pool_usado`;
  const r = await sbFetch(url);
...
async function incrementarPool(assinaturaId, poolAtual) {
  const r = await sbFetch(`/assinaturas?id=eq.${assinaturaId}`, {
    method: 'PATCH',
    headers: { Prefer: 'return=minimal' },
    body: JSON.stringify({ pool_usado: poolAtual + 1 })
  });
  if (!r.ok) console.error('incrementarPool falhou:', r.data);
}
...
// (Frequências sempre chama com false — livre para qualquer assinante no ciclo).
...
export default async function handler(req, res) {
  ...
  const { action, email, nome, estado, intencao, dados, id, geometrias_recentes, frequencias_recentes } = req.body || {};
  const geoRecentes = Array.isArray(geometrias_recentes) && geometrias_recentes.length > 0 ? geometrias_recentes : [];
  const hzRecentes = Array.isArray(frequencias_recentes) && frequencias_recentes.length > 0 ? frequencias_recentes : [];
  ...
  if (action === 'gerar') {
    ...
    const acesso = await verificarAcesso(email, false, 'frequencias');
    if (!acesso.permitido) {
...
${geoRecentes.length > 0 ? `GEOMETRIAS JÁ USADAS COM ${firstName} (evite repetir — se o contexto atual exigir uma delas, escolha a segunda opção mais adequada): ${geoRecentes.join(', ')}` : ''}
${hzRecentes.length > 0 ? `FREQUÊNCIAS JÁ USADAS COM ${firstName} (evite repetir — se o contexto atual exigir uma delas, escolha a segunda opção mais adequada): ${hzRecentes.map(h => h + ' Hz').join(', ')}` : ''}
...
REGRAS:
- REGRA CRÍTICA: analise genuinamente o estado de ${firstName} e escolha a frequência e geometria mais adequadas para o que ele está vivendo agora — a escolha deve ser sempre contextual, nunca por padrão ou familiaridade. 528 Hz e Flor da Vida são apenas duas das 9 frequências e 20 geometrias disponíveis; use outras quando o estado de ${firstName} indicar isso
- NÃO REPETIÇÃO: as geometrias e frequências listadas como PROIBIDO acima foram usadas recentemente com ${firstName}. Se a mais adequada ao contexto atual for uma delas, escolha a segunda opção mais adequada — nunca repita as recentes, mas a escolha deve sempre ser contextual
...
Responda APENAS em JSON válido, sem markdown, sem aspas tipográficas, sem quebras de linha dentro dos valores — use \\n para separar parágrafos:
{
  ...
  "leitura": {
    ...
    try {
      ...
      // Cobra (debita crédito de trial, se for o caso) assim que a IA confirma
      // que vai responder. Assinante dentro do ciclo não é cobrado (Frequências
      // não usa pool, é livre pra qualquer plano ativo).
