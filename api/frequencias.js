...
export default async function handler(req, res) {
  ...
  const { action, email, nome, estado, intencao, dados, id, geometrias_recentes } = req.body || {};
...
${geoRecentes.length > 0 ? `GEOMETRIAS RECENTES DE ${firstName} (PROIBIDO REPETIR — escolha uma diferente de todas estas): ${geoRecentes.join(', ')}` : ''}
...
- NÃO REPETIÇÃO: as geometrias listadas como PROIBIDO acima foram usadas recentemente com ${firstName}${geoRecentes.length > 0 ? ` (${geoRecentes.join(', ')})` : ''}. Se a geometria mais adequada ao contexto atual for uma delas, escolha a segunda opção mais adequada — nunca repita as recentes, mas a escolha deve sempre ser contextual
