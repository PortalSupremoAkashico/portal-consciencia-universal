  ...
  "leitura": {
    ...
    try {
      const resp = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model: process.env.ANTHROPIC_MODEL_SONNET || 'claude-sonnet-4-6', max_tokens: 3200, messages: [{ role: 'user', content: prompt }] })
      });
      if (!resp.ok) { const e = await resp.text(); return res.status(500).json({ error: 'Erro na API.', detail: e }); }
      const apiData = await resp.json();
      console.log('[freq] apiData.stop_reason:', apiData.stop_reason, '| content items:', apiData.content?.length);
      let text = apiData.content?.[0]?.text || '';
      console.log('[freq] text length:', text.length, '| preview:', text.slice(0,200));
      const match = text.match(/\{[\s\S]*\}/);
      if (!match) return res.status(500).json({ error: 'Resposta inválida.', textLength: text.length, stopReason: apiData.stop_reason, preview: text.slice(0,500) });
