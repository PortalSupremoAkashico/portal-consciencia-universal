...
export default async function handler(req, res) {
  ...
  const { action, email, nome, estado, intencao, dados, id, geometria_anterior } = req.body || {};
...
${geometria_anterior ? `GEOMETRIA DA SESSÃO ANTERIOR (PROIBIDA — NÃO REPITA): ${geometria_anterior}` : ''}
...
- NÃO REPETIÇÃO: se a geometria indicada pelo contexto de ${firstName} for a mesma da sessão anterior${geometria_anterior ? ` (${geometria_anterior})` : ''}, escolha a segunda opção mais adequada para o estado atual — nunca repita a mesma geometria em sessões consecutivas, mas a escolha deve sempre ser contextual
