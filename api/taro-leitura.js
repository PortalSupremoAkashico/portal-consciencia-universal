...
export default async function handler(req, res) {
  ...
  const { nome, sexo, cartas, tiragem, pergunta } = req.body || {};
  ...
  const prompt = `Tarólogo akáshico. Leitura direta e rica para ${firstName}.

NAIPES: CHAMAS=Fogo, CÁLICES=Água, CRISTAIS=Ar, ESTRELAS=Terra. Arcanos Maiores=karma.
CONSULENTE: ${firstName}. PERGUNTA: "${perguntaReal}". TIRAGEM: ${tiragem || 'Leitura Livre'}.

CARTAS:
${cartasTexto}

FORMATO OBRIGATÓRIO (siga exatamente, sem markdown):
...
INTERPRETACAO: [parágrafo 1: significado da carta e aplicação à pergunta "${perguntaReal}" para ${firstName} nesta posição]
[parágrafo 2: mensagem dos Registros e conselho para ${firstName}]`).join('\n\n')}

SINTESE: [parágrafo 1: padrão geral]
[parágrafo 2: caminho para ${firstName}]

ACAO: [ação concreta para ${firstName}]`;
