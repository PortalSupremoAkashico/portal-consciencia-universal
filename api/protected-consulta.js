...
export default async function handler(req, res) {
  ...
  if (!session) {
    ...
    // Primeiro acesso sem sessão: apresenta o portal antes do login.
    // Cookie existente, porém inválido/expirado: leva ao login e preserva o retorno.
    const destination = rawToken
      ? '/login?redirect=%2Fconsulta&sessao=expirada'
      : '/?acesso=restrito';
