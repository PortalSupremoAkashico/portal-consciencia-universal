...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    // ── SALVAR URL DO ÁUDIO DE UMA ABA ──
    if (action === 'salvar_audio_url') {
      const { consulta_id, key, audio_url } = req.body;
      if (!consulta_id || !key || !audio_url) {
        return res.status(400).json({ error: 'consulta_id, key e audio_url são obrigatórios.' });
      }

      // Busca o audio_url atual para fazer merge (guarda URL por aba)
      const getRes = await supabaseFetch(
        `/consultas?id=eq.${consulta_id}&select=audio_url`,
        { method: 'GET' }
      );
      let audioUrls = {};
      if (getRes.ok && Array.isArray(getRes.data) && getRes.data.length > 0) {
        try { audioUrls = JSON.parse(getRes.data[0].audio_url || '{}'); } catch {}
      }
      audioUrls[key] = audio_url;

      const upd = await supabaseFetch(`/consultas?id=eq.${consulta_id}`, {
        method: 'PATCH',
        body: JSON.stringify({ audio_url: JSON.stringify(audioUrls) })
      });

      return res.status(upd.ok ? 200 : 500).json(upd.ok ? { success: true } : { error: 'Erro ao salvar áudio.' });
    }
