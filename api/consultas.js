...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    // ── SALVAR URL DO ÁUDIO ──
    // ── SALVAR ÁUDIO DE UMA ABA (upload verificado, substitui o upload direto do navegador) ──
    if (action === 'salvar_audio') {
      const { email, consulta_id, key, audioBase64 } = req.body;
      if (!email || !consulta_id || !key || !audioBase64) {
        return res.status(400).json({ error: 'Dados obrigatórios ausentes.' });
      }

      // Confirma que essa consulta realmente pertence a esse e-mail antes de gravar
      ...
      if (sizeBytes > 15 * 1024 * 1024) {
        return res.status(400).json({ error: 'Áudio muito grande.' });
      }
      ...
      const upResp = await fetch(`${SUPABASE_URL}/storage/v1/object/consultas-audio/${fn}`, {
        method: 'PUT',
        headers: {
          apikey: SUPABASE_KEY,
          Authorization: `Bearer ${SUPABASE_KEY}`,
      ...
      if (!upResp.ok) {
        ...
        console.error('Erro ao salvar áudio da consulta:', upResp.status, errTxt.slice(0, 300));
        return res.status(500).json({ error: 'Erro ao salvar áudio.' });
      }

      return res.status(200).json({
        success: true,
        url: `${SUPABASE_URL}/storage/v1/object/public/consultas-audio/${fn}`
      });
    }
