...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'salvar') {
      ...
      const insert = await supabaseFetch('/consultas', {
        ...
        body: JSON.stringify({
          ...
          estado,
          pergunta,
          revelation:          sections?.revelation          || '',
          earth_future:        sections?.earthFuture         || '',
          other_civilizations: sections?.evolution || sections?.otherCivilizations || '',
          technology_future:   sections?.technologyFuture    || '',
    ...
    // ── SALVAR URL DO ÁUDIO ──
    // ── OBTER URL ASSINADA PARA UPLOAD DE ÁUDIO (evita o limite de tamanho de requisição
    // da própria Vercel — o áudio vai direto do navegador pro Supabase Storage) ──
    if (action === 'obter_url_upload_audio') {
      const { email, consulta_id, key } = req.body;
      if (!email || !consulta_id || !key) {
        return res.status(400).json({ error: 'Dados obrigatórios ausentes.' });
      }

      // Confirma que essa consulta realmente pertence a esse e-mail antes de autorizar o upload
      const dono = await supabaseFetch(
        `/consultas?id=eq.${consulta_id}&email=eq.${encodeURIComponent(email)}&select=id`,
        { method: 'GET' }
      );
      if (!dono.ok || !Array.isArray(dono.data) || dono.data.length === 0) {
        return res.status(403).json({ error: 'Consulta não encontrada para este usuário.' });
      }

      const chaveArquivo = String(key).replace(/[^a-zA-Z0-9]/g, '_');
      const emailArquivo = String(email).replace(/[^a-zA-Z0-9]/g, '_');
      const fn = `${emailArquivo}_${consulta_id}_${chaveArquivo}.mp3`;

      const signResp = await fetch(`${SUPABASE_URL}/storage/v1/object/upload/sign/consultas-audio/${fn}`, {
        method: 'POST',
        headers: {
          apikey: SUPABASE_KEY,
          Authorization: `Bearer ${SUPABASE_KEY}`,
      ...
      if (!signResp.ok) {
        ...
        console.error('Erro ao assinar upload de áudio:', signResp.status, errTxt.slice(0, 300));
      ...
      return res.status(200).json({
        ...
        publicUrl: `${SUPABASE_URL}/storage/v1/object/public/consultas-audio/${fn}`
      });
    }

    if (action === 'salvar_audio_url') {
      const { consulta_id, audio_url } = req.body;
      if (!consulta_id || !audio_url) {
        return res.status(400).json({ error: 'consulta_id e audio_url obrigatórios.' });
      }
      const upd = await supabaseFetch(`/consultas?id=eq.${consulta_id}`, {
        method: 'PATCH',
        body: JSON.stringify({ audio_url })
      });
      return res.status(upd.ok ? 200 : 500).json(upd.ok ? { success: true } : { error: 'Erro ao salvar URL do áudio.' });
    }
